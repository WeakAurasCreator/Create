...
async def download_image(session, spell_id, icon_filename):
    """Download and save spell icon asynchronously."""
...
async def process_data():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for spell_id, spell_data in spells.items():
            tasks.append(handle_spell(session, spell_id, spell_data, spellmisc, manifest))

        await asyncio.gather(*tasks)

async def handle_spell(session, spell_id, spell_data, spellmisc, manifest):
    ...
    # Save spell JSON data asynchronously
    ...
    if icon_filename:
        await download_image(session, spell_id, icon_filename)
